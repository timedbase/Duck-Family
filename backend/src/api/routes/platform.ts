...
async function fetchAllPaged<T extends { id: string }>(
  ...
  let lastId = "";
  for (;;) {
    const data = await querySubgraph<Record<string, T[]>>(chain, queryFor(lastId), { ...variables, lastId });
...
export default function createPlatformRouter(chain: ChainSlug) {
  ...
  router.get("/hook", async (_req, res) => {
    try {
      ...
      const [owner, platformWallet, ctoFee, hookFeeDefaultBps] = await Promise.all([
        ...
        publicClient.readContract({ address: addr.DUCK_HOOK, abi: DUCK_HOOK_ABI, functionName: "HOOK_FEE_DEFAULT_BPS" }),
      ]);
      res.json({ address: addr.DUCK_HOOK, owner, platformWallet, ctoFee: (ctoFee as bigint).toString(), hookFeeDefaultBps: Number(hookFeeDefaultBps) });
    } catch (err) {
      res.status(502).json({ error: err instanceof Error ? err.message : String(err) });
    }
  });
  ...
  router.get("/curve", async (_req, res) => {
    try {
      ...
      const [platformWallet, platformToken, creationFee, v4PositionManager, v4Singleton, v4Permit2, v4Hook] = await Promise.all([
        publicClient.readContract({ address: addr.DUCK_INCUBATION, abi: DUCK_INCUBATION_ABI, functionName: "platformWallet" }),
        publicClient.readContract({ address: addr.DUCK_INCUBATION, abi: DUCK_INCUBATION_ABI, functionName: "platformToken" }),
        publicClient.readContract({ address: addr.DUCK_INCUBATION, abi: DUCK_INCUBATION_ABI, functionName: "creationFee" }),
        publicClient.readContract({ address: addr.DUCK_INCUBATION, abi: DUCK_INCUBATION_ABI, functionName: "v4PositionManager" }),
        publicClient.readContract({ address: addr.DUCK_INCUBATION, abi: DUCK_INCUBATION_ABI, functionName: "v4Singleton" }),
        publicClient.readContract({ address: addr.DUCK_INCUBATION, abi: DUCK_INCUBATION_ABI, functionName: "v4Permit2" }),
        publicClient.readContract({ address: addr.DUCK_INCUBATION, abi: DUCK_INCUBATION_ABI, functionName: "v4Hook" }),
      ]);
      res.json({
        address: addr.DUCK_INCUBATION,
        platformWallet,
        platformToken,
        creationFee: (creationFee as bigint).toString(),
  ...
  router.get("/raise", async (_req, res) => {
    try {
      ...
      const [platformWallet, platformToken, campaignFee, v4PositionManager, v4Singleton, v4Permit2, v4Hook] = await Promise.all([
        publicClient.readContract({ address: addr.DUCK_RAISE, abi: DUCK_RAISE_ABI, functionName: "platformWallet" }),
        publicClient.readContract({ address: addr.DUCK_RAISE, abi: DUCK_RAISE_ABI, functionName: "platformToken" }),
        publicClient.readContract({ address: addr.DUCK_RAISE, abi: DUCK_RAISE_ABI, functionName: "campaignFee" }),
        publicClient.readContract({ address: addr.DUCK_RAISE, abi: DUCK_RAISE_ABI, functionName: "v4PositionManager" }),
        publicClient.readContract({ address: addr.DUCK_RAISE, abi: DUCK_RAISE_ABI, functionName: "v4Singleton" }),
        publicClient.readContract({ address: addr.DUCK_RAISE, abi: DUCK_RAISE_ABI, functionName: "v4Permit2" }),
        publicClient.readContract({ address: addr.DUCK_RAISE, abi: DUCK_RAISE_ABI, functionName: "v4Hook" }),
      ]);
      res.json({
        address: addr.DUCK_RAISE,
        platformWallet,
        platformToken,
        campaignFee: (campaignFee as bigint).toString(),
        dex: { positionManager: v4PositionManager, singleton: v4Singleton, permit2: v4Permit2, hook: v4Hook },
      });
    } catch (err) {
      res.status(502).json({ error: err instanceof Error ? err.message : String(err) });
  ...
  router.get("/quote-tokens", async (req, res) => {
    ...
    const target = family === "launcher" ? addr.DUCK_LAUNCHER : addr.DUCK_INCUBATION;
  ...
  router.get("/quote-assets", async (_req, res) => {
    try {
      const publicClient = getPublicClient(chain);
      const results = await Promise.all(
        addr.RAISE_DEFAULT_QUOTE_ASSETS.map((t) =>
          publicClient.readContract({
            address: addr.DUCK_RAISE,
            abi: DUCK_RAISE_ABI,
      ...
      const assets = [
        { address: NATIVE_ADDRESS, symbol: addr.nativeSymbol, allowed: true },
        ...addr.RAISE_DEFAULT_QUOTE_ASSETS.map((t, i) => ({ address: t.address, symbol: t.symbol, allowed: results[i] })),
      ];
      res.json(assets);
    } catch (err) {
      res.status(502).json({ error: err instanceof Error ? err.message : String(err) });
  ...
  // (see DEFAULT_QUOTE_TOKENS) plus native currency -- used to label the
  // raw, mixed-currency fee totals below by their real quote asset rather
  // than blending them into one fabricated USD figure (fee-claim entities
  // don't carry a resolved USD value the way TokenHourData.volumeUsd does).
  const QUOTE_ASSET_META: Record<string, { symbol: string; decimals: number }> = {
    [NATIVE_ADDRESS]: { symbol: addr.nativeSymbol, decimals: 18 },
    ...Object.fromEntries(addr.DEFAULT_QUOTE_TOKENS.map((t) => [t.address.toLowerCase(), { symbol: t.symbol, decimals: 6 }])),
  };
  ...
  router.get("/stats", async (_req, res) => {
    ...
    try {
      ...
      // Crowdlaunch contributions are raw native currency, not USD-resolved
      // volume -- reported separately rather than blended into the same
      // percentage split as dex/curve (which IS real USD), to avoid implying
      // a false apples-to-apples comparison the way a single fabricated pie
      // would. Field name kept as -Eth (not renamed to the chain-neutral
      // -Native) since the interface reads this exact key and isn't being
      // touched for Arc yet -- see StatsPage.jsx.
