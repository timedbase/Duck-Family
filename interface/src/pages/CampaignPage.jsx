        ...
        <div style={cs("display:flex;flex-direction:column;gap:16px;min-width:0")}>
          <div style={cs("border:2px solid var(--ink);background:var(--card);box-shadow:3px 3px 0 var(--ink)")}>
            ...
            <div style={cs("padding:20px;border-bottom:2px solid var(--ink)")}>
              <div style={cs(`border:2px solid var(--ink);background:${camp.noteBg};color:${camp.noteFg};padding:14px 16px;font-size:13px;line-height:1.55;margin-bottom:20px`)}>{camp.note}</div>
              <div style={cs("display:flex;gap:3px;height:26px;border:2px solid var(--ink);background:var(--card);padding:3px")}>
                {camp.ticks.map((c, i) => <div key={i} style={cs(`flex:1;background:${c}`)}></div>)}
              </div>
            ...
            <div style={cs("display:flex;flex-wrap:wrap")}>
              {camp.facts.map((f, i) => (
                <div key={i} style={cs("flex:1;min-width:150px;padding:13px 16px;border-right:2px solid var(--ink)")}>
          ...
          <div style={cs("border:2px solid var(--ink);background:var(--card);box-shadow:3px 3px 0 var(--ink);overflow-x:auto")}>
            ...
            <div style={cs("display:grid;min-width:520px;grid-template-columns:1.4fr 1fr 1fr;gap:14px;padding:10px 18px;border-bottom:2px solid var(--ink);background:var(--paper);font-family:'DM Mono',monospace;font-size:10px;letter-spacing:.14em;color:var(--mute)")}>
        ...
        <div style={cs(`display:flex;flex-direction:column;gap:16px;${v.isMobile ? "" : "position:sticky;top:80px"}`)}>
          <div style={cs("border:2px solid var(--ink);background:var(--card);box-shadow:3px 3px 0 var(--ink)")}>
            ...
            <div style={cs("padding:18px")}>
              {camp.canContribute && (
                <div style={cs("display:flex;align-items:stretch;border:2px solid var(--ink);background:var(--paper);margin-bottom:16px")}>
                  ...
                  <span style={cs("padding:0 14px;border-left:2px solid var(--ink);display:flex;align-items:center;font-family:'DM Mono',monospace;font-size:13px")}>ETH</span>
                </div>
              )}
              <button onClick={v.submitCampaignAction} disabled={v.txPending} style={cs(`width:100%;padding:16px;border:2px solid var(--ink);background:${camp.ctaBg};color:${camp.ctaFg};font-size:15.5px;font-weight:700;cursor:pointer;box-shadow:3px 3px 0 var(--ink)`)}>{v.txPending ? "Confirming…" : camp.cta}</button>
              <div style={cs("font-size:12px;color:var(--mute);line-height:1.55;margin-top:14px")}>{camp.ctaNote}</div>
            </div>
          </div>

          <div style={cs("border:2px solid var(--ink);background:var(--card);box-shadow:3px 3px 0 var(--ink)")}>
            <div style={cs("padding:11px 15px;border-bottom:2px solid var(--ink);background:var(--paper);font-family:'DM Mono',monospace;font-size:10px;letter-spacing:.14em;color:var(--mute)")}>ESCROW &amp; CUSTODY</div>
          ...
          <div style={cs("border:2px solid var(--ink);background:var(--card);box-shadow:3px 3px 0 var(--ink)")}>
            <div style={cs("padding:11px 15px;border-bottom:2px solid var(--ink);background:var(--paper);font-family:'DM Mono',monospace;font-size:10px;letter-spacing:.14em;color:var(--mute)")}>RESOLUTION PATH</div>
            {camp.timeline.map((t, i) => (
              <div key={i} style={cs("display:flex;gap:12px;padding:13px 15px;border-bottom:1px solid var(--soft)")}>
                <span style={cs(`width:11px;height:11px;border:2px solid var(--ink);background:${t.on ? "var(--lime)" : "var(--paper)"};margin-top:3px;flex:none`)}></span>
                <div>
