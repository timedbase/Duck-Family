...
const LAUNCH_STEPS = [
  ...
  { n: "4", k: "Sign one transaction", v: "The factory clones the token, wires the shared anti-MEV hook, and — for instant launches — opens the V4 pool and locks LP in the same call.", bg: "var(--paper)" },
];
...
const TRADE_STEPS = [
  ...
  { n: "4", k: "Sign and settle", v: "The hook rejects a second buy/sell pair from the same address in the same block, so a sandwich attempt reverts at the pool. Sells on a V4 pool are skimmed a 2% hook fee, paid to the creator.", bg: "var(--paper)" },
];
...
const HOW_NOTE = {
  launch: { k: "Liquidity is locked, permanently", v: "Every family ends the same way: a full-range V4 position minted into DuckLocker with no withdrawal path. Creators keep the fee stream, never the liquidity." },
  trade: { k: "What you are actually trading against", v: "Before migration you trade the curve, and your counterparty is the contract. After migration it is an ordinary Uniswap V4 pool whose LP sits in DuckLocker and can never be pulled — so the liquidity you see cannot disappear." },
};
  ...
  return (
    <div style={cs("max-width:1000px;margin:0 auto")}>
      ...
      <div style={cs("display:grid;grid-template-columns:repeat(auto-fit,minmax(280px,1fr));gap:12px;margin-bottom:18px")}>
        {steps.map((s, i) => (
          <div key={i} style={cs("border:1px solid var(--line);background:var(--card);border-radius:14px;padding:20px;display:flex;flex-direction:column")}>
      ...
      <div>
        ...
        <div style={cs("display:flex;flex-direction:column;gap:10px")}>
          {FAQS.map((f, i) => (
            <div key={i} style={cs("border:1px solid var(--line);background:var(--card);border-radius:14px;padding:16px 18px")}>
