  ...
  return (
    <div>
      <div style={cs("border:1px solid var(--line);border-radius:10px;background:var(--card);margin-bottom:18px;overflow:hidden")}>
        <div style={cs("padding:24px 22px 20px;border-bottom:1px solid var(--line)")}>
      ...
      <div style={cs("border:1px solid var(--line);border-radius:10px;background:var(--card);margin-bottom:16px;overflow-x:auto")}>
        <div style={cs("padding:13px 18px;border-bottom:1px solid var(--line);font-size:15px;font-weight:700;letter-spacing:-.02em")}>Holdings</div>
        <div style={cs("display:grid;min-width:700px;grid-template-columns:1.6fr .8fr .8fr 1fr 90px;gap:14px;padding:10px 18px;border-bottom:1px solid var(--line);background:var(--paper);font-family:'DM Mono',monospace;font-size:10px;letter-spacing:.14em;color:var(--mute)")}>
          <span>TOKEN</span><span style={cs("text-align:right")}>BALANCE</span><span style={cs("text-align:right")}>% SUPPLY</span><span style={cs("text-align:right")}>VALUE</span><span></span>
        </div>
        {held.length === 0 && (
        ...
        {held.map((h, i) => {
          ...
          return (
            <div key={i} style={cs("display:grid;min-width:700px;grid-template-columns:1.6fr .8fr .8fr 1fr 90px;gap:14px;padding:12px 18px;border-bottom:1px solid var(--soft);align-items:center;font-family:'DM Mono',monospace;font-size:12.5px")}>
              <div style={cs("display:flex;align-items:center;gap:11px;min-width:0")}>
                <div style={cs(`width:26px;height:26px;border:1px solid var(--line);border-radius:8px;background:${co ? co.famBg : "var(--card)"};color:${co ? co.famFg : "var(--ink)"};flex:none;display:flex;align-items:center;justify-content:center;font-size:10px;font-weight:700;font-family:'Space Grotesk',sans-serif`)}>{co ? co.initials : "??"}</div>
              ...
              <span style={cs("text-align:right")}>{balanceTokens.toLocaleString(undefined, { maximumFractionDigits: 2 })}</span>
              <span style={cs("text-align:right;color:var(--mute)")}>{sharePct != null ? sharePct.toFixed(sharePct < 1 ? 3 : 1) + "%" : "—"}</span>
              <span style={cs("text-align:right")}>{co ? usdOrQuote(valueUsd, value, co.quote) : "—"}</span>
              <div style={cs("text-align:right")}><button onClick={() => co && v.openToken(co.id)} style={cs("padding:6px 12px;border:1px solid var(--line);border-radius:7px;background:var(--card);font-size:11.5px;font-weight:600;cursor:pointer;font-family:'Space Grotesk',sans-serif")}>Open</button></div>
            </div>
          );
        })}
      </div>
      ...
      <div style={cs("display:grid;grid-template-columns:repeat(auto-fit,minmax(340px,1fr));gap:16px")}>
        <div style={cs("border:1px solid var(--line);border-radius:10px;background:var(--card);overflow:hidden")}>
          <div style={cs("padding:13px 18px;border-bottom:1px solid var(--line);font-size:15px;font-weight:700;letter-spacing:-.02em")}>Claims &amp; refunds</div>
        ...
        <div style={cs("border:1px solid var(--line);border-radius:10px;background:var(--card);overflow:hidden")}>
          <div style={cs("padding:13px 18px;border-bottom:1px solid var(--line);display:flex;align-items:center;justify-content:space-between;gap:10px")}>
          ...
          {created.map((t, i) => {
            ...
            return (
              <div key={i} style={cs("display:flex;align-items:center;gap:14px;padding:14px 18px;border-bottom:1px solid var(--soft)")}>
                ...
                <button onClick={() => v.claimCreatorFees(t.id)} style={cs("padding:7px 13px;border:1px solid var(--line);border-radius:7px;background:var(--card);font-size:11.5px;font-weight:600;cursor:pointer;flex:none")}>Claim fees</button>
              </div>
            );
          })}
        </div>
